...
def _construct_offline_pooler(corpus_jobs: list) -> PoolingService:
    ...
    try:
        return PoolingService(corpus_jobs)
    finally:
...
def _chunk_identity() -> dict:
    rows = (
        CareerJobChunk.objects.filter(active=True, source="vietjobs")
        .order_by("source_job_id", "chunk_index", "chunk_id")
        .values_list(
            "source_job_id", "chunk_id", "chunk_index", "job_title", "section", "content",
            "location_key", "experience_level", "employment_type", "category_key",
        )
    )
    digest = hashlib.sha256()
    count = rows.count()
    for values in rows.iterator(chunk_size=4000):
        (
            source_job_id, chunk_id, chunk_index, job_title, section, content,
            location_key, experience_level, employment_type, category_key,
        ) = values
        payload = json.dumps(
            {
                "source_job_id": source_job_id, "chunk_id": chunk_id, "chunk_index": chunk_index,
                "job_title": job_title, "section": section, "content": content,
                "location_key": location_key, "experience_level": experience_level,
                "employment_type": employment_type, "category_key": category_key,
            },
            ensure_ascii=False,
            sort_keys=True,
            separators=(",", ":"),
...
def _corpus_snapshot(*, seed: int = DEFAULT_RANDOM_SEED, max_pool: int = 80) -> dict:
    ...
    chunk = _chunk_identity()
    membership_sha = sha256_text("\n".join(sorted(db_job_ids)))
    ...
    provenance_value = os.environ.get("CAREER_RAG_EMBEDDING_PROVENANCE_PATH")
    provenance = embedding_provenance_contract(
        backend_root=BACKEND_ROOT,
        corpus_membership_sha256=membership_sha,
        corpus_chunks_sha256=chunk["sha256"],
        forbidden_derived_metadata_present=not leakage["passed"],
    ...
    if raw_rows != 48_092:
        blockers.append(f"frozen source row count drift: expected 48092, got {raw_rows}")
    if not corpus_jobs:
    ...
    if len(db_job_ids) != 47_097:
        blockers.append(f"indexed job count drift: expected 47097, got {len(db_job_ids)}")
    if chunk["count"] != 152_379:
        blockers.append(f"indexed chunk count drift: expected 152379, got {chunk['count']}")
    ...
    if not embedding_provenance_is_freeze_safe(provenance):
        blockers.append(f"embedding provenance is {provenance['status']}: {provenance.get('reason', '')}")
    ...
    topic_shape_ok = (
        ...
        and all(
            len(queries_by_topic[topic.topic_id]) == 3
    ...
    pooling_report: dict
    pooling_policy = POOLING_POLICY_RRF
    pooling_reason = "real offline pooling audit unavailable"
    pooling_policy_changed = False
    try:
        pooler = _construct_offline_pooler(corpus_jobs)
        pooling_report = audit_pool_coverage_offline(
            pooler, topics, queries_by_topic, depths=POOL_DEPTHS, max_pool=max_pool
        )
        pooling_policy, pooling_reason, pooling_policy_changed = choose_pooling_policy(pooling_report, decision_depth=20)
        pooling_report.update({
            "pooling_policy_changed": pooling_policy_changed,
            "pooling_policy": pooling_policy,
            "pooling_policy_reason": pooling_reason,
        })
    except Exception as exc:  # noqa: BLE001
        pooling_report = {
    ...
    corpus_manifest = {
        ...
        "chunking": {
            ...
            "chunking_source_sha256": provenance.get("chunking_source_sha256"),
        },
    ...
    return {
        ...
        "truncation": truncation, "provenance": provenance, "topics": topics, "queries": queries,
        ...
        "pooling": pooling_report, "pooling_policy": pooling_policy,
        "pooling_policy_changed": pooling_policy_changed, "pooling_reason": pooling_reason,
...
def run_construction_preflight(
    *, output_dir: Path = DEFAULT_PREFLIGHT_OUTPUT_DIR, seed: int = DEFAULT_RANDOM_SEED, max_pool: int = 80
) -> dict:
    ...
    if snapshot.get("provenance", {}).get("status") != "VERIFIED_CLEAN" and not any("embedding provenance" in item for item in blockers):
        blockers.append("embedding provenance is not VERIFIED_CLEAN")
...
def _build_benchmark_into(output_dir: Path, *, judge_model: str, seed: int, pool_depth: int, max_pool: int) -> dict:
    ...
    free = run_construction_preflight(output_dir=reports_dir, seed=seed, max_pool=max_pool)
    ...
    pool_rows: list[dict] = []
    for topic in topics:
        candidates = pooler.pool_topic(topic.topic_id, queries_by_topic[topic.topic_id], depth=pool_depth, max_pool=max_pool, pooling_policy=free["pooling_policy"])
    ...
    audit_report = run_audit(topics=topics, qrels=all_qrels, nuggets=nuggets, controls=[control.to_dict() for control in controls])
    ...
    manifest = BenchmarkManifest(
        ...
        configuration={
            ...
            "pooling_policy_changed": free["pooling_policy_changed"], "pooling_policy_reason": free["pooling_reason"],
            ...
            "embedding_provenance_status": free["provenance"]["status"], "git_head": git_sha, "git_dirty_at_freeze": git_dirty,
        },
