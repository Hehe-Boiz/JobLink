...
class VietJobsFieldAwareE3Evaluator:
    ...
    def __init__(self) -> None:
        ...
        self.category_to_docs: dict[
            str,
            set[str],
        ] = defaultdict(set)

        self.location_to_docs: dict[
            str,
            set[str],
        ] = defaultdict(set)

        self.category_phrase_to_key: dict[
            str,
            str,
        ] = {}
    ...
    def run(self) -> dict:
        ...
        for index, query_item in enumerate(
            ...
            e2_1_metric = evaluate_ranking(
                e2_1_ranked,
                relevant_doc_ids,
            )

            e3_metric = evaluate_ranking(
                e3_ranked,
                relevant_doc_ids,
            )
        ...
        payload = {
            "config": {
                ...
                "retrieval_inputs": (
                    ...
                    "+ corpus metadata only"
                ),
                "uses_query_filters_for_retrieval": (
                    False
                ),
    ...
    def _build_corpus_indexes(
        ...
        for record in self.source.iter_records():
            ...
            if category:
                self.category_to_docs[
                    category
                ].add(
                    doc_id
                )

                self.category_phrase_to_key[
                    _humanize_category(
                        category
                    )
                ] = category

            for location in _parse_locations(
                record.location_key
            ):
                self.location_to_docs[
                    location
                ].add(
                    doc_id
                )
        ...
        if not tokenized_corpus:
            raise RuntimeError(
                "No skill corpus available."
            )

        bm25_doc_id_to_index = {
            doc_id: index
            for index, doc_id
            in enumerate(bm25_doc_ids)
        }
    ...
    def _parse_query(
        ...
        if normalized.startswith(
            ...
            if SKILL_CUE in body:
                ...
                category_phrase = (
                    category_phrase.strip()
                )

                category_key = (
                    self.category_phrase_to_key.get(
                        category_phrase
                    )
                )
    ...
    def _e2_1_rank(
        ...
        *,
        ...
        parsed: ParsedCareerQuery,
    ) -> list[str]:
        if not parsed.skill_query:
            return dense_ranked

        bm25_ranked = self._search_bm25(
    ...
    def _e3_rank(
        ...
        *,
        ...
        # CATEGORY + LOCATION:
        # exact metadata candidate set is already the semantic intent.
        # Rank candidates by their existing E1 dense order when possible,
        # then fill remaining positions deterministically.
        if (
            parsed.category_key
            and parsed.location_key
            and not parsed.skill_query
        ):
            allowed = (
                self.category_to_docs.get(
                    parsed.category_key,
                    set(),
                )
                & self.location_to_docs.get(
                    parsed.location_key,
                    set(),
                )
            )
        ...
        # CATEGORY + SKILL:
        # failure analysis showed category dominates dense and hurts skill.
        # First enforce category, then rank by skill-only BM25.
        if (
            parsed.category_key
            and parsed.skill_query
        ):
            allowed = self.category_to_docs.get(
                parsed.category_key,
                set(),
            )
            ...
            return (
                bm25_ranked,
                "category_filter+skill_bm25",
            )

        # LOCATION + SKILL:
        # E2.1 showed dense + BM25 complement each other.
        # Restrict both branches to the parsed location and fuse.
        if (
            parsed.location_key
            and parsed.skill_query
        ):
            allowed = self.location_to_docs.get(
                parsed.location_key,
                set(),
            )
            ...
            # If the saved dense top-20 contains too few docs after
            # location filtering, BM25 remains the main branch.
            fused = _rrf_fuse(
                dense_filtered,
                bm25_ranked,
                top_k=MAX_RANK,
            ...
            return (
                fused,
                "location_filter+dense+skill_bm25",
            )
    ...
    def _metadata_filtered_dense(
        ...
        *,
        ...
        # The saved E1 artifact only has top-20 dense results.
        # To avoid pretending we have deeper dense scores, remaining
        # allowed candidates are appended deterministically by doc_id.
        # This makes the limitation explicit rather than using qrels.
